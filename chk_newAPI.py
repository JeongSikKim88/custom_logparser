...
with open('logs/json/20200304_newAPI.json') as data_file:
    data = json.load (data_file)
    #jdata=json.dump(data, indent = 4, sort_keys=True)

# req=data['request_record']

### data list : request_count, transfer_bps, transfer_byte ###
...
for i in range(0,287):
    ...
    sum += int(json.dumps(data['transfer_bps'][i]['mkr-l2m.ncupdate.com'], indent = 4, sort_keys=True))
    # print (json.loads(json.dumps(data['transmission_record'][i]['time'])),json.loads(json.dumps(data['transmission_record'][i]['total'])))
    print (json.loads(json.dumps(data['transfer_bps'][i]['time'])),json.loads(json.dumps(data['transfer_bps'][i]['mkr-l2m.ncupdate.com'])))
    # print (sum)
print (" sum : ",sum)